...
For direct-use distribution the same shell ships as a second flavor (`scripts/build.ts --frozen`, macOS first): the harness is cloned and built with its full dependency graph (it resolves some imports through cross-package hoisting, so a prod-only prune breaks runtime resolution), the tree is copied with symlinks kept and rewritten to relative paths (Tauri's resource packer drops links, so the payload is injected into the bundle post-build with rsync), source maps and build caches are stripped, identical files duplicated by pnpm's hoisted layout are hard-link-deduplicated (~1.2 GB), and an official node binary (SHA-256 verified) is frozen beside them. The distribution is a tar.gz — tar stores deduped hard links once, where a zip would embed both copies and double the size. At startup the shell detects `frozen/`, spawns the orchestrator with the bundled node (`DSH_FROZEN_ROOT` points `boot.mjs` at the payload), and the backend starts with zero git/pnpm/network contact; the splash footer is switched to say so via a `mode` event. The tradeoff is explicit: a frozen build never updates itself — shipping a new archive is the upgrade path — and the source launcher remains the always-fresh flavor.
